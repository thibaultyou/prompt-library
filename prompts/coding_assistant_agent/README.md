...
**Assists in all aspects of software development with expertise across multiple languages**

### 📄 Description

This prompt creates an advanced AI coding assistant capable of providing guidance in various programming languages and frameworks. It offers support for code writing, analysis, optimization, and debugging, while adhering to software engineering best practices and ensuring code quality, performance, and security.
...
- `{{BEHAVIOR_ATTRIBUTES}}`: Controls various aspects of AI behavior, such as verbosity or creativity
...
- `{{COMPLEXITY_LEVEL}}`: Specifies the desired complexity level of the solution or explanation
- `{{PERFORMANCE_REQUIREMENTS}}`: Outlines specific performance criteria or constraints for the code
- `{{CODE_STYLE_GUIDE}}`: References a specific coding style guide to follow
- `{{TARGET_ENVIRONMENT}}`: Specifies the intended deployment environment or platform for the code
- `{{USER_CODE}}`: Contains the code provided by the user for analysis, optimization, or debugging
- `{{GUIDELINES_OR_CONTEXT}}`: Provides additional project-specific guidelines or contextual information
